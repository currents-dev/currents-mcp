...
Use this when the work is done and someone needs to see it. To debug a test that is failing, use `currents-get-context` instead. To prove a change that no test covers, drive the browser yourself — that is `browser-evidence`.
...
| ---------------------- | ------------------------------------------------- | ------------------------------------------ |
| Trace digest           | What the test did and where it broke              | `currents-create-trace-link` → `digest`    |
...
- `currents-create-trace-link` in your tool list, for step 3. It is the one evidence tool gated on both the `results:read` scope and the organization's evidence sharing, so the rest of the list says which is missing: with `currents-get-test-evidence` beside it, the scope is granted and evidence sharing is off, and the trace is a `trace.zip` you download from the manifest and attach — it opens at https://trace.playwright.dev, but you cannot read or quote it. With neither tool, the credential is missing `results:read`, every evidence tool went with it, and there is nothing to fall back to: say that and stop.
...
- **`currents-create-trace-link` is not in your tools**: look for `currents-get-test-evidence`. If it is there, the scope is granted and the organization does not have evidence sharing. If it is not, the credential is missing `results:read` and the whole evidence path is gone with it.
